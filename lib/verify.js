...
const reservedClaimNames = [
  ...
  'principal',
  'attribute_certs',
  'cdi', // cert digest info
  'scope', // attr claim ID
  'cdn'  // attr claim display name
];
...
function flattenAttributeCertClaims(obj) {
  ...
  _.each(Object.keys(obj.attributeCertClaims), function(c) {
    _.extend(obj.idpClaims, obj.attributeCertClaims[c]);
  });
  delete obj.attributeCertClaims;
}
...
function verify(browserid, args, cb) {
  ...
  jwcrypto.cert.verifyBundle(
    ...
    function(issuer, next) {
      ...
      if (Array.isArray(userClaims.attribute_certs)) {
        obj.attributeCertClaims = [];
        _.each(userClaims.attribute_certs, function(attrCert, index) {
          verifyAttributeCert(attrCert, now, ultimatePublicKey,
            ...
            if (extClaims) {
              if (obj.attributeCertClaims[attributeCertClaims.scope]) {
                return cb("multiple attribute certificates with same id");
              }
              obj.attributeCertClaims[attributeCertClaims.scope] = extClaims;
            }
            if (index === _.size(userClaims.attribute_certs) - 1) {
