...
function verifyAttributeCert(jwt, now, publicKey, certHash, certIssuer, cb) {
...
function flattenAttributeCertClaims(obj) {
  ...
  _.each(Object.keys(obj.attrCertClaims), function(c) {
    _.extend(obj.idpClaims, obj.attrCertClaims[c]);
  });
      ...
      if (Array.isArray(userClaims['attr-certs'])) {
        ...
        _.each(userClaims['attr-certs'], function(attrCert, index) {
          verifyAttributeCert(attrCert, now, ultimatePublicKey,
            ...
            if (extClaims) {
              if (obj.attrCertClaims[attrCertClaims.id]) {
                return cb("multiple attribute certificates with same id");
              }
              obj.attrCertClaims[attrCertClaims.id] = extClaims;
            }
